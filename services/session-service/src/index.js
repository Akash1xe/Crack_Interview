...
async function getSession(id) {
  ...
  const files = await pool.query(
    'SELECT id,session_id,file_id_ref,typed_code,typed_path,submitted_at FROM session_files WHERE session_id=$1 ORDER BY id',
    [id]
  );
...
app.post('/sessions', async (req, res, next) => {
  try {
    ...
    const snapshot = {
      ...
      files: project.files.map(({ id, path, reference_code, language, order_index }) => ({
        id, path, reference_code, language, order_index
      }))
    };
  ...
  try {
    ...
    if (session.status === 'in_progress') {
      ...
      if (Date.now() >= expiresAt) {
        await pool.query(
          "UPDATE sessions SET status='expired',ended_at=COALESCE(ended_at,now()) WHERE id=$1",
          [req.params.id]
        );
...
app.patch('/sessions/:id/files/:fileId', async (req, res, next) => {
  try {
    ...
    if (!result.rowCount) return fail(res, 404, 'Session file not found');
      ...
      for (const file of submittedFiles) {
        await client.query(
          `UPDATE session_files
           SET typed_code=$1,typed_path=$2,submitted_at=now()
           WHERE session_id=$3 AND file_id_ref=$4`,
          [file.typed_code || '',file.typed_path || '',req.params.id,file.file_id_ref]
        );
      }
      await client.query(
        "UPDATE sessions SET status='submitted',ended_at=now() WHERE id=$1",
        [req.params.id]
      );
      await client.query('COMMIT');
    } catch (error) {
